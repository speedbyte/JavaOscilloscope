...
public class GPSDataEventListener implements SerialPortEventListener {
    ...
    private static GPSData locationData = new GPSData();
        ...
        try {
            if(event.getEventType() == SerialPortEvent.DATA_AVAILABLE) {
                ...
                try {
                    ...
                    boolean GPGGA_next_round=false;
                    boolean GPGSA_next_round=false;
                    if(!reader.ready()) {
                        setConnection(false);
                    }
                    while (reader.ready()) {
                        ...
                        if (type.equals("$GPGGA")) {
                            locationData.setGpgga(currLine);
                        	GPGGA_next_round = true;
                        }else if (type.equals("$GPRMC")) {
                            locationData.setGprmc(currLine);
                            GPRMC_next_round = true;
                        } else if (type.equals("$GPGSA")) {
                            locationData.setGpgsa(currLine);
                            GPGSA_next_round = true;
                        }
                        if(GPRMC_next_round && GPGGA_next_round) {
                            break;
                        }
                    }
                    if(GPRMC_next_round || GPGGA_next_round) {
